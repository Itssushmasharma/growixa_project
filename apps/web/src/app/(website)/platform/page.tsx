...
import { STAGES, STATUS } from "@/content/website/stages";
import { hueVars } from "@/components/website/layout/section";
import {
  PageHero,
  Sec,
  Head,
...
const STATUS_ROWS = [
  {
    id: "send",
    works: "Campaigns, sequences, templates, warm-up, suppression, analytics",
    next: "SMS and WhatsApp",
  },
  {
    id: "manage",
    works: "Contacts, dedup, custom fields, imports, consent and suppression",
    next: "Dynamic segments",
  },
  {
    id: "find",
    works: "Sourcing and email verification, invite-only",
    next: "Open beta, phone enrichment",
  },
  { id: "create", works: "Email and social drafting, brand voice", next: "Approval workflow" },
  {
    id: "qualify",
    works: "Nothing yet — this one is honest vapour",
    next: "First signals, Q4 2026",
  },
];
  ...
  return (
    <>
      <PageHero
        ...
        actions={
          <>
            <Button as={Link} href="/platform/send">
              Start with campaigns
            </Button>
            <Button as={Link} href="/platform/find" variant="glass">
              See stage one
            </Button>
          </>
        }
      ...
      <Sec hue="create">
        ...
        <div className={own.handoff}>
          {STAGES.map((s) => (
            <Link key={s.id} href={s.path} className={own.node} style={hueVars(s.hue)}>
