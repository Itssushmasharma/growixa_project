...
import styles from "./word-rotator.module.css";

const WORDS = ["GTM", "marketing", "sales", "outbound", "growth"] as const;
const INTERVAL = 2600;
const SWAP = 330;
...
export default function WordRotator() {
  ...
  useEffect(() => {
    const currentWord: string = WORDS[index] ?? "GTM";
    measure(currentWord);
    const onResize = () => measure(WORDS[index] ?? "GTM");
