...
| OQ-013 | Given [DEC-GRX-029](DECISIONS.md) (Razorpay, dual-currency, top-up concept confirmed): what are the actual plan tiers/names/prices (per currency), which Razorpay billing primitive to use (Subscriptions API vs. self-managed Orders + own renewal tracking), what does a top-up purchase — AI generation credits, email/social send volume, something else — actually buy and how is it priced/consumed, and which specific features are gated per tier? | `GRX-SAAS-004` (Billing), `GRX-SAAS-006`, `GRX-SAAS-009` | OPEN — product owner to provide; blocks implementation start on `GRX-SAAS-004` (schema/design work may proceed on the shape only, per DEC-GRX-029) |
