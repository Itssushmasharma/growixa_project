...
permission-check dependency), `GRX-AUDIT-001` (audit log module), and `GRX-TEST-001`
(backend test foundation) are `DONE`. `GRX-TEST-002` (frontend test foundation),
`GRX-COMPANY-001` (company profile + brand settings), `GRX-AUTH-002` (password hashing +
login/logout), and `GRX-USER-001` (internal user invitation + acceptance) are all `READY`.
Per explicit user direction, `GRX-COMPANY-001` is next; per
...
1. Per explicit user direction, `GRX-COMPANY-001` (company profile + brand settings) is
   next. `GRX-TEST-002`, `GRX-AUTH-002`, and `GRX-USER-001` are also dependency-ready in
   the meantime — see [MASTER_TASK_TRACKER.md](MASTER_TASK_TRACKER.md).
