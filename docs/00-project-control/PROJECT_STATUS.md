...
revocation), and `GRX-USER-001` (internal user invitation + acceptance) are `DONE`.
`GRX-TEST-002` (frontend test foundation), `GRX-COMPANY-002` (company settings screen),
`GRX-AUTH-005` (password reset flow), `GRX-FOUND-008` (dashboard shell), and `GRX-USER-002`
...
1. `GRX-AUTH-002` → `GRX-AUTH-003` → `GRX-USER-001` chosen per explicit user direction
   (P0, backend, "most needed" at each step — rotation/reuse-detection closed a gap
   `GRX-AUTH-002` left open; invitation was the only real way to add any user besides the
   still-unbuilt `seed_first_admin` CLI). All three now `DONE`. Pick one of `GRX-AUTH-005`
   (password reset flow), `GRX-TEST-002` (frontend test foundation), `GRX-COMPANY-002`,
