...
test foundation), and `GRX-COMPANY-001` (company profile + brand settings) are `DONE`.
`GRX-TEST-002` (frontend test foundation), `GRX-AUTH-002` (password hashing + login/logout),
`GRX-USER-001` (internal user invitation + acceptance), and `GRX-COMPANY-002` (company
settings screen, newly unblocked) are all `READY`. Per
...
1. The user-specified sequence (`GRX-AUDIT-001` → `GRX-TEST-001` → `GRX-COMPANY-001`) is
   complete. Pick one of `GRX-TEST-002` (frontend test foundation), `GRX-AUTH-002`
   (password hashing + login/logout), `GRX-USER-001` (internal user invitation +
   acceptance), or `GRX-COMPANY-002` (company settings screen, frontend) next — see
