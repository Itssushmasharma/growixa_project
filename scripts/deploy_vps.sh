...
echo "Step 6: Cleaning up unused dangling Docker images..."
...
if curl -sf http://localhost:8000/health > /dev/null; then
    ...
    # Exit non-zero so cron, CI and any wrapper see a failed deploy. Previously this
    # printed a warning and still exited 0, reporting success on a broken deploy.
    exit 1
fi
